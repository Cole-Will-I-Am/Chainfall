...
// Chainfall design system — the single source of truth for the brand (see the Chainfall brand
...
enum Haptics {
    ...
    /// One tick per cascade wave, escalating with chain depth (juice spec).
    static func pop(depth: Int) {
        let style: UIImpactFeedbackGenerator.FeedbackStyle = depth >= 5 ? .rigid : depth >= 3 ? .medium : .light
        let g = UIImpactFeedbackGenerator(style: style)
        g.impactOccurred(intensity: min(1.0, 0.55 + 0.11 * Double(depth)))
    }
}
#else
enum Haptics {
